...
@section('konten')
    ...
    <style>
        ...
        .teacher-card {
            ...
            cursor: pointer;
            height: 100%;
            /* Memaksa card mengisi tinggi kolom */
            display: flex;
            flex-direction: column;
            min-height: 320px;
            /* Menjaga agar card tidak terlalu pendek */
        }
        ...
        .teacher-placeholder-img {
            ...
            height: 100%;
            object-fit: cover;
            /* Agar icon memenuhi kotak seperti foto asli */
        }
        ...
        .teacher-info {
            ...
            text-align: center;
            flex-grow: 1;
            /* Membuat area info mengambil sisa ruang */
            display: flex;
            flex-direction: column;
            justify-content: center;
        }
                ...
                <div class="modal-content p-4 p-md-5">
                    ...
                    <div class="modal-body p-0 text-start">
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-md-7">
                                <div class="row">
                                    <div class="col-6 mb-3"><span class="detail-label">NIK</span>
                                        <p class="detail-text">{{ $guru->nik ?? '-' }}</p>
                                    </div>
                                    <div class="col-6 mb-3"><span class="detail-label">NUPTK</span>
                                        <p class="detail-text">{{ $guru->nuptk ?? '-' }}</p>
                                    </div>
                                    <div class="col-6 mb-3"><span class="detail-label">JENIS KELAMIN</span>
                                        <p class="detail-text">{{ $guru->jk == 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                    </div>
                                    <div class="col-6 mb-3"><span class="detail-label">KONTAK</span>
                                        <p class="detail-text">{{ $guru->no_hp ?? '-' }}</p>
                                    </div>
                                </div>
                                <div class="mb-3"><span class="detail-label">EMAIL</span>
                                    <p class="detail-text text-break">{{ $guru->email ?? '-' }}</p>
                                </div>
                                <div class="mb-3"><span class="detail-label">TEMPAT & TANGGAL LAHIR</span>
                                    <p class="detail-text">{{ $guru->tempat_lahir }},
                                        {{ $guru->tanggal_lahir ? \Carbon\Carbon::parse($guru->tanggal_lahir)->translatedFormat('d F Y') : '-' }}
                                    </p>
                                </div>
                                <div class="mt-3 pt-3 border-top">
                                    <span class="detail-label">BIDANG KEAHLIAN / MAPEL</span>
                                    <p class="detail-text">{{ $guru->mapel }}</p>
                                    <span class="detail-label">PENEMPATAN</span>
                                    <p class="detail-text">{{ $guru->penempatan }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
